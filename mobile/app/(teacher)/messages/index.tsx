...
export default function TeacherConversationsList() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      <View className="px-4 pt-4 pb-3 flex-row items-center justify-between border-b border-gray-100">
        ...
        <TouchableOpacity
        onPress={() => router.push('/(teacher)/messages/new' as never)}
      ...
      {conversations.length === 0 && !isLoading ? (
        ...
        <FlashList
          ...
          renderItem={({ item }: { item: ConversationDto }) => (
            <ConversationRow item={item} primaryColor={colors.primary} />
          )}
        />
      )}
    </SafeAreaView>
...
function ConversationRow({
  ...
  return (
    <TouchableOpacity
      ...
      style={{ backgroundColor: hasUnread ? primaryColor + '0D' : 'white' }}
      onPress={() => router.push(`/(teacher)/messages/${item.id}` as never)}
      activeOpacity={0.7}
    >
      <View
      ...
      >
        <Text className="text-lg font-bold" style={{ color: VITANA_COLORS.textSecondary }}>

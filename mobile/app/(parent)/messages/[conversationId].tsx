...
import { communicationApi, type MessageDto, type ConversationDto } from '@/api/endpoints/communication';
...
export default function ParentMessageThread() {
  ...
  const headerName = conversation?.participantName ?? 'Teacher';

  // ── Fetch messages ────────────────────────────────────────────────────────
  const { data, fetchNextPage, isLoading, isFetchingNextPage } = useInfiniteQuery({
    queryKey: ['messages', conversationId],
    queryFn: ({ pageParam = 1 }) =>
      communicationApi.getMessages(conversationId, pageParam as number),
            ...
            return (
              <View className={`mb-2 flex-row ${isMine ? 'justify-end' : 'justify-start'}`}>
                <View
                  ...
                  style={{ backgroundColor: isMine ? colors.primary : VITANA_COLORS.surface }}
                >
                  <Text
                  ...
                  <View className="flex-row items-center justify-end mt-1 gap-1">
                    <Text
                      className="text-xs"
                      style={{
                        color: isMine ? 'rgba(255,255,255,0.7)' : VITANA_COLORS.textSecondary,
                      }}
                    >
                      {timeStr}
                    </Text>
                    {isMine && (
                      <Feather
                        ...
                        size={11}
                        color={
                          item.status === 'read' ? '#93c5fd' : 'rgba(255,255,255,0.65)'
                        }
                      />
                    )}
                  </View>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View className="bg-white border-t border-gray-100 px-4 py-3 flex-row items-end gap-2">
          <TextInput
            ...
            multiline
            maxLength={2000}
            placeholder={isConnected ? 'Reply…' : 'No connection'}
          ...
          >
            {sendMutation.isPending ? (
              ...
              <Feather
                name="send"
                size={17}
                color={canSend ? 'white' : VITANA_COLORS.textSecondary}
              />
            )}
          </TouchableOpacity>
        </View>

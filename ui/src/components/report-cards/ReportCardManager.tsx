...
export function ReportCardManager() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t('reportCards.deleteSuccess'));
      queryClient.invalidateQueries({ queryKey: ['report-cards'] });
    },
    onError: () => toast.error(t('reportCards.deleteFailed')),
  });

  const distributeMutation = useMutation({
    mutationFn: (id: string) => reportCardDocumentsApi.markDistributed(id),
    onSuccess: () => {
      toast.success(t('reportCards.distributeSuccess'));
      queryClient.invalidateQueries({ queryKey: ['report-cards'] });
    },
  });

  const handleDelete = (rc: ReportCardDocumentDto) => {
    if (!window.confirm(t('reportCards.deleteConfirm').replace('{name}', rc.studentName))) return;
    ...
    <div className="space-y-4">
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
          {[
            { label: t('reportCards.statGenerated'), value: stats.totalGenerated,   color: 'text-blue-600'  },
            { label: t('reportCards.statDistributed'), value: stats.totalDistributed, color: 'text-green-600' },
            { label: t('reportCards.statDraft'), value: stats.totalDraft,          color: 'text-amber-600' },
            { label: t('reportCards.statPassed'), value: stats.totalPassed,        color: 'text-emerald-600'},
            { label: t('reportCards.statFailed'), value: stats.totalFailed,        color: 'text-red-600'   },
            { label: t('reportCards.statAvg'), value: `${(stats.classAveragePercentage ?? 0).toFixed(1)}%`, color: 'text-purple-600' },
          ].map(s => (
      ...
      <Tabs defaultValue="list">
        <TabsList className="mb-2">
          <TabsTrigger value="list" className="flex items-center gap-1.5">
            <Layers className="h-3.5 w-3.5" />{t('reportCards.tabList')}
          </TabsTrigger>
          <TabsTrigger value="generate" className="flex items-center gap-1.5">
            <FileText className="h-3.5 w-3.5" />{t('reportCards.tabGenerate')}
          </TabsTrigger>
          <TabsTrigger value="bulk-download" className="flex items-center gap-1.5">
            <Download className="h-3.5 w-3.5" />{t('reportCards.tabBulkDownload')}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="list">
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            ...
            <Select value={termFilter || 'all'} onValueChange={v => { setTermFilter(v === 'all' ? '' : v); setPage(1); }}>
              ...
              <SelectContent>
                <SelectItem value="all">{t('reportCards.allTerms')}</SelectItem>
                {TERMS.map(trm => <SelectItem key={trm} value={trm}>{trm}</SelectItem>)}
              </SelectContent>
            </Select>
            <Select value={statusFilter || 'all'} onValueChange={v => { setStatusFilter(v === 'all' ? '' : v); setPage(1); }}>
              ...
              <SelectContent>
                ...
                <SelectItem value="Generated">{t('reportCards.statusGenerated')}</SelectItem>
                <SelectItem value="Distributed">{t('reportCards.statusDistributed')}</SelectItem>
                <SelectItem value="Draft">{t('reportCards.statusDraft')}</SelectItem>
              </SelectContent>
            </Select>
          ...
          <Card>
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>{t('reportCards.colStudent')}</TableHead>
                    <TableHead>{t('common.class')}</TableHead>
                    <TableHead>{t('reportCards.colTerm')}</TableHead>
                    <TableHead>{t('reportCards.colBoard')}</TableHead>
                    <TableHead className="text-center">%</TableHead>
                    <TableHead className="text-center">{t('reportCards.colGrade')}</TableHead>
                    <TableHead className="text-center">{t('reportCards.colResult')}</TableHead>
                          ...
                          <div className="flex items-center justify-end gap-1">
                            ...
                            {rc.status !== 'Distributed' && (
                              <Button
                                ...
                                title={t('reportCards.markDistributed')}
            ...
            {listData && listData.totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t text-sm text-muted-foreground">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" size="sm" disabled={page >= listData.totalPages} onClick={() => setPage(p => p + 1)}>{t('common.next')} →</Button>
                </div>
              </div>
            )}
          </Card>
...
function StatusBadge({ status }: { status: string }) {
  ...
  const map: Record<string, { label: string; className: string; icon: React.ReactNode }> = {
    Distributed: { label: t('reportCards.statusDistributed'), className: 'bg-green-100 text-green-700', icon: <CheckCircle2 className="h-3 w-3" /> },
    Generated:   { label: t('reportCards.statusGenerated'),   className: 'bg-blue-100 text-blue-700',   icon: <FileText className="h-3 w-3" /> },
    Draft:       { label: t('reportCards.statusDraft'),        className: 'bg-gray-100 text-gray-600',   icon: <Clock className="h-3 w-3" /> },
  };
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-sm flex items-center gap-2">
          ...
          {t('reportCards.bulkTitle')}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            <span className="text-xs font-medium text-muted-foreground">{t('common.academicYear')}</span>
            <div className="h-9 px-3 rounded-md border flex items-center text-sm font-medium">{academicYear}</div>
          </div>
          <div className="space-y-1">
            <span className="text-xs font-medium text-muted-foreground">{t('common.term')}</span>
        ...
        <DownloadDialog
          ...
          trigger={
            <Button className="w-full">
              ...
              {t('reportCards.bulkDownloadBtn')} ({term} {academicYear})
            </Button>
          }
        />
      </CardContent>

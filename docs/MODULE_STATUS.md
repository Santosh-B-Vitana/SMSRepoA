...
**Last Updated:** May 30, 2026 (Session 12) | **Project:** SMSRepoA (Release Candidate)  
...
| **Academics** | ✅ Production Ready | 70/70 | Classes (school-wide, no year filter), subjects from live API, teacher search bar in assign dialog; **Class Settings Board Defaults Panel (May 30 2026)** — Settings tab shows board's effective grade scale + pass% as read-only reference card, amber "Class Override Active" badge when class-level grade tiers are defined, passing% field seeds from board default on first create |
...
| **Examinations** | ✅ Production Ready | ✅ | Marks entry, grade reports, rank generation; **Hall Tickets** (bulk-generate with prefix, per-class filter, CSV export, print); **Co-Scholastic Grading** (A+→E grade per area per term, area CRUD); **Promote Exam Structure** — added May 2026; **Exam Marks Entry (admin + staff)** — end-to-end fixed May 13, 2026; **Exams tab in My Classes** — shows all class exams (not just assigned); **ExamResultsTab Class + Section filter (May 16 2026)** — replaced status dropdown with Class + Section cascading filter, loads results per class/section; **Board-Aware Grading (May 30 2026)** — exam results, analytics, and report summary all use board-specific grade scale and dynamic grade distribution |
| **Board Configuration** | ✅ Production Ready | ✅ | **New — Session 12 (May 30 2026).** 15 curriculum boards seeded (CBSE, ICSE, ISC, IB, CAIE, 10 state boards). Schools configure boards via Settings → Board Configuration; one set as default. Override chain: Class Grade Tiers → School Custom → Board Default → 33% fallback. `PATCH /api/board/school-boards/{id}/overrides` for per-school pass%/scale overrides. Build: 0 errors. |
